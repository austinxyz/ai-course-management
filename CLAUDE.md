...
**`NULL` 互不相等，所以唯一索引会"建了但没挡住"**（enrollment-core group 1）。
`unique (student_email, course_id, session_id)` 挡不住两条都是"未定场次"的记录 ——
Postgres 里 `NULL != NULL`，两行都被收下且互不冲突，而它们表示的是同一件事，
批量导入每跑一遍就多一条。要拆成两条 partial index（`where session_id is not null`
与 `where session_id is null`）。**验证只能靠真实插入两次**；migration 跑通、索引存在
都不构成证据。本次是把索引删掉、确认测试真的变红，才算数。

**外键管不到"属于哪门课"这类跨表关系约束**（enrollment-core group 2，evaluator BLOCK）。
`enrollments.session_id` 只能指向 `course_sessions.id`，DB 层无法表达"这一场必须属于
这条报课的课程"。不在边界上挡的话，A 课的场次能挂到 B 课的报课上，而**状态是从"所属场次"
派生的** —— 这条报课会按一个毫不相干的日期显示已完成。凡是"两个外键之间还有关系"的地方，
DB 只保证各自存在，不保证彼此相关，那道校验只能写在应用层。

**显式 `null` 的拒绝要按列的可空性分别决定，不能一刀切 `"*"`**（enrollment-core group 2，同一次 BLOCK）。
`EnrollmentUpdate` 里 `enrolled_at`/`note`/`status` 背后是 NOT NULL 列，显式 null 只能是误用，
必须挡；但 `session_id` 本来就可空，**显式 null 是合法的"清空场次"**，也是补课流程唯一的表达方式。
既有的 `_reject_explicit_null` 挂在 `field_validator("*")` 上，照抄过来会把正常功能一起挡死。

**设计稿的示例数字会被抄进生产代码，而且不止一处**（enrollment-core group 4）。
学员详情里同时藏着两处：归档确认写死「他的 **2 条报课、4 份作业**和互动记录」，
以及"待设计"占位块写死「报课记录 2 条 / 作业提交 **4 / 6** / 互动记录 最近 7 天 1 条」。
对每个学员都显示同一个数字 —— **永远相同的数字比没有数字更糟，它看起来像信息**。
报课真做出来之后，那个假的"报课记录 2 条"就贴在真区块上方，一真一假并排。
规则：能力不存在时**不写数字**（写"还没做"），能力存在后数字必须来自数据。

**清表 fixture 要跟着外键长，漏一张就让全套测试红在不相干的地方**（enrollment-core group 6）。
`empty_course_tables` 原本只清课程三表；有了 `enrollments` 之后，本地库里只要留着一条报课
（截图用的、验收剩下的），删 `course_sessions` 就撞外键 —— **129 个测试全部 error，
且报错落在某个与报课毫不相干的课程测试的 setup 阶段**。新增引用表时，
除了想"它引用了谁"，还要回头改那张清空顺序表：先引用方，后被引用方。

**读接口没有 `ORDER BY` 时，「第一条」根本不是可依赖的定位方式**（enrollment-backfill 生产验收，改错了用户的数据）。
`roster-editing` 记过"按主键重新定位目标"，但那条说的是**改名会让列表重排**；
这次更基础：`GET /api/enrollments` 压根没有排序，`rows[0]` 每次可能是不同的行。
验收脚本用"S1 的第一条报课"当操作目标，取到的是**用户手工补录的那条**（不是脚本刚建的），
把它从 6/28 挪到了 6/07。两条记录长得几乎一样，改错了看不出来。
规则：**验收脚本只操作自己刚建的记录**（按 id，或按只有自己会写的标记如 `source == "derived"` 筛），
且全程按 id 操作，不在中途重新"找"一遍。

**一次只读的 dry-run 会因为控制台编码而崩，而崩的样子像"数据有问题"**（enrollment-backfill 生产 dry-run）。
脚本输出整个是中文，Windows 控制台默认 cp1252，`print` 在第一行就抛 `UnicodeEncodeError`。
本地测试全绿（pytest 捕获的是内存流），只有真在终端跑才暴露。
凡是**输出非 ASCII 的命令行脚本**，都写成 `sys.stdout.buffer.write(line.encode("utf-8"))`，
并留一个"模拟 cp1252 流"的测试。`sys.stdout.reconfigure(encoding="utf-8")` 也行，
但它对已被重定向的流不生效。

**"他碰巧不在名单里"不是排除，是巧合**（enrollment-backfill）。
讲师自己的测试提交没有被导入，原因只是他不在学员库里——哪天他被加进名单，
那些记录就会**静默**变成报课。要排除就写成显式的 `--exclude`，
并与"待补建的人"分开列：前者是"本来就不该算"，后者是"补建后要补上"，处置相反。

**204 没有 body，而写请求的公共壳里一句 `res.json()` 就能把成功变成失败**（enrollment-backfill 上线后用户报「删除不成功」）。
`backendWrite` 结尾无条件 `return res.json()`，`DELETE` 返回 204 时解析空 body 抛异常 →
Server Action 捕获 → 返回"没删掉"、`revalidatePath` **不执行**、那一行留在屏幕上，
**而记录其实已经删掉了**。症状与"真的删不掉"一模一样，只有刷新页面才分得出。
凡是有 204 的端点，公共壳必须显式跳过解析（`if (res.status === 204) return undefined`）。
更一般地：**把"解析响应体"写死在所有写请求的共同路径上，就等于假设每个写请求都有响应体。**

**唯一约束只挡得住它定义的那种重复，"业务上是同一件事"要在写入前自己判断**（enrollment-backfill，真实数据里撞到）。
`(学员, 课程, 场次)` 唯一 + "同一(学员,课程)最多一条未定场次"两条索引都在，
却挡不住"一条有场次 + 一条未定场次"——**两条都合法**，而业务上那是同一件事。
于是倒推脚本给一名已有手工报课（6/28）的学员又建了一条未定场次的同课报课。
结构约束是最后一道，不是唯一一道：**写入前要问的是"这件事是不是已经被某条记录表达了"，
而不是"数据库会不会拒绝我"**。

**`overflow-hidden`（为圆角）+ 可压缩的 flex 子项 = 内容被静默裁掉，且哪儿都没有滚动条**（enrollment-backfill 上线后用户报「看不到全部记录」）。
表格外框写了 `overflow-hidden rounded-token`，它被 flex 父容器压到 555px 后**裁掉**了 1226px 的表格；
外层那个 `overflow-y-auto` 因此**看不到任何溢出**，于是不出滚动条 —— 记录不是"看不见"，是**够不着**。
外框要 `flex-none`（按内容撑开），让溢出冒到滚动容器上。
**jsdom 没有布局，这类缺陷单测量不出来**（只能钉类名），必须**在真实浏览器里用足够多的数据**验一次；
本次的数据量差别就是 4 条（看着好好的）与 22 条（一半够不着）。
...
**在页面内部计时**：`performance.now()` + `MutationObserver` 等目标节点出现，
用原生 `element.click()` 触发。这样量到的才是应用的时间，不是测量工具的时间。
...
线程整段在等网络。凡是"慢在哪"的判断，先抓一份 profile 再说；
`(idle)` 占满就是在等 I/O，与"客户端渲染慢"是相反的结论和相反的修法。

**噪声与效应同量级时，三次采样会骗人；留一个没改过的接口当对照**（同上）。
优化上线后第一轮 3 次采样显示 `/api/students`（**我根本没碰**）慢了 54ms —— 
噪声就有 ±50ms，而我要测的效应也是这个量级。改成 7 次采样、并看**最小值**
（排队与抖动只会让某次变慢，不会让它变快），信号才干净。
对照组是关键：没有它，我会把 `courses` 的 −69ms 当成真实收益写进报告。

**"每次请求发几条 SQL"要写成断言，否则它只会悄悄变差**（性能优化，`test_query_roundtrips.py`）。
实测每次数据库往返 ≈ 61ms（Render → Supabase），固定开销 ≈ 103ms —— 三个接口的耗时
都落在 `103 + 61 × 查询数` 这条线上。所以往返次数**就是**用户感知的延迟。
但它退化时没有任何征兆：有人顺手加一行 `session.exec(...)`，功能测试全绿，
只是每次切页面慢 61ms。用 SQLAlchemy 的 `before_cursor_execute` 数一数，钉成断言。

**设计稿是视觉基准，但实现有意偏离的地方以实现为准 —— 偏离不是待修的 bug**（2026-07-31 用户确认）。
`docs/superpowers/specs/mocks/*-course-enrollment-design.dc.html` 来自 claude.ai/design，
是**单向**的：改动没有 sync 回设计稿。所以稿子与实现不一致时有两种可能，方向相反 ——
要么实现漂移了（该改实现），要么是有意的决定（该留着）。
**分辨的唯一依据是最新那份 `.dc.html` 头注释里的「已知偏离」清单**，它逐条记了偏离内容、
理由与拍板日期。重新导入设计稿后第一件事是读它，别照着稿子把这些"修回去"。
当前在册的偏离：学员详情栏 560px + 字段两列（稿子 358px 单列）、报课页只读（稿子有导入向导与两个徽标）。

**FastAPI 对不认识的查询参数是静默忽略的，于是"新加的开关"在旧构建上等于没加**（homework 生产同步，我把一次 dry-run 跑成了真写入）。
客户端加了 `?dry_run=true` 就以为安全，而那一刻线上还是旧构建 —— 参数被无视，16 条成绩真写进了生产库。
更要命的是我**验证过**新构建：用「空 body 返回 404 还是 422」判断端点在不在。可端点上一次 push 就有了，
**两个构建都返回 422** —— 判据必须是**只有新构建才有的可观察差异**（比如新参数真的生效），
而不是"某个更早就存在的东西还在"。凡是加了新参数/新字段的部署，验证要打那个**新东西本身**。

**`jsonb` 不保证对象键顺序，所以"顺序有含义"的数据只能存数组**（homework 决策 1）。
Postgres 存储时按键长度、再按字节序重排。实测 10 个真实评分项名过一遍 jsonb 对象，
`A1 A2 A3 B1 B2 B3 C1 C2 D1 D2` 被打散成 `A2 A3 B2 B3 D1 D2 A1 C1 C2 B1` —— A/B/C/D 的分组彻底毁掉，
而**页面照常渲染、没有任何报错**。存 `[{item, score}]` 数组。
配套的测试断言必须比较**有序序列**：断言集合相等或转成 dict 比较，对这个缺陷完全全盲。

**dry-run 不能实现成"照常写、最后 rollback"**（homework，第一版就这么写的）。
那样 dry-run 的正确性取决于**调用方**的事务边界：测试里 session 绑在外层事务上，
`rollback()` 把 pytest fixture 自己的清表操作一并撤销了，17 行本该不可见的数据浮回来，
症状是"dry-run 居然写进去了"。**不写就是不写** —— 分类照算，只是不往 session 里 add 对象。
比"写了再撤"少一整类关于事务语义的假设。

**做变异测试时别用 `git checkout <file>` 还原 —— 未提交的工作会一起没**（homework group 3）。
基线已提交、当前改动还没提交时，`git checkout` 把文件还原到 HEAD，正在写的实现就没了，
而且**不报错**（"Updated 1 path from the index"），下一条命令才发现功能凭空消失。
变异前把文件复制到 scratchpad，变异后从副本还原。

**本地演示数据与测试夹具会在 `@example.com` 上撞，因为两者都只能用它**（homework 验收前）。
`empty_course_tables` 有意不清 `students`，理由写着"测试用 @example.com，撞不上"——
但为截图/验收造的本地数据按隐私规则**也只能用** @example.com，所以撞是结构性的，不是运气差。
撞上的症状是**四个不相干的测试红在 setup 阶段**（`duplicate key ... students_pkey`）。
