...
```bash
--exclude austin.xyz@gmail.com
```

可重复。`session1/grades.csv` 里讲师本人的邮箱出现**两次**，两行的姓名列还不一样
（是测试提交）。它们目前没进库，唯一原因是他不在学员表里 —— 那是**巧合，不是排除**。
他哪天被加进学员名单，那两条测试提交就会静默变成真实作业成绩，还带着假名字。

排除掉的行会在报告里列出来：悄悄少几行与"数据本来就没有"分不出来。
