...
- [x] 5.5 上线 —— 无 schema 变更，前后端各自部署，顺序不敏感
- [x] 5.6 **脚本 dry-run 打生产** —— 与预测完全一致：22 条（S1 14 + S2 8）、跳过 5 人、
      `session4` 带原因跳过。人工确认后 `--apply`，新建 22 条。
      过程中补了两处：dry-run 在 Windows 控制台因 cp1252 崩（输出整个是中文，改为写
      stdout.buffer 并显式 UTF-8）；讲师本人的提交改为 `--exclude` **显式排除**，
      而不是依赖"他碰巧不在学员库里"
- [x] 5.7 **重跑一次** —— 新建 0 条、已存在 22 条，不报错退出。幂等由数据库保证
- [x] 5.8 生产验收 —— `/enroll` 显示 **23 条**：22 条脚本倒推 + **1 条用户此前手工补录的**。
      筛选 S1 = 15（14 倒推 + 1 手工）、S2 = 8。22 条未定场次
- [x] 5.9 生产验收 —— 课程页 S1「另有 14 人未定场次」、S2「另有 8 人未定场次」，
      且两门课的场次卡片上**都不显示**已报人数（全为 0）。
      **这一条最容易被误读成"导入没成功"**，要专门确认
- [x] 5.10 生产验收 —— 指派一条后：未定场次 14 → 13、该场已报 0 → 1；改回未定场次完全复原。
      **第一次做这条时改错了记录**：用"S1 的第一条"取目标，而读接口没有 `ORDER BY`，
      取到的是用户手工补录的那条（把它从 6/28 挪到了 6/07）。已按邮箱+来源重新定位并改回。
      重做时只操作 `source == "derived"` 且未定场次的记录，并全程按 id 操作
