...
三条待验项的结论，以及一个过程中踩到的假信号：

- **Render 健康检查与 401** —— 后端对未授权请求返回 401 后，Render 未判服务不健康。401 是正常响应，
  服务本身在跑，符合预期。
- **移动端 Basic Auth** —— 未单独实测（用户以电脑为主、手机只读）。留待实际使用中观察。
- **中文 realm 会把 401 变成 500**（测试发现，非预期）—— HTTP header 值是 ByteString，
  `WWW-Authenticate: Basic realm="学员管理系统"` 在构造响应时直接抛 `TypeError`。realm 必须 ASCII。
  这条只有真正执行到 401 分支才会暴露，靠阅读代码看不出来。

- **假信号：端口被占时 uvicorn 静默退出，探针打到旧进程。** 验证本地后端时一度全部返回 200，
  看着像 middleware 完全没生效；实际是上一轮的 uvicorn 仍占着 8000，新进程绑定失败后退出，
  curl 打的是**旧代码**。日志里 `[Errno 10048] only one usage of each socket address` 才是真相。
  这与决策 #1 的风险同源——**都是"看起来在跑、其实跑的不是你以为的东西"**。验证认证类改动时，
  必须确认打的是新进程/新构建，否则结论完全不可信。
