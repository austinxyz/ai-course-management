...
## 已知偏离（与 `2026-07-31-homework-upload-mocks.html`）

设计稿是布局与语气，不是取值域。下面这条是**有意**不照做的，其余都对齐了。

### 预览屏顶部不显示「10 个分项列」

稿子的那行是「按 UTF-8 读取 · 18 行 → 去重后 17 行 · **10 个分项列**」。
前两段都做了（第二段实现成 `共 18 行 → 可用 17 行`，因为丢行有两种原因——
重复提交被顶掉、以及没有邮箱——只说"去重后"在后一种情况下是假话）。

分项列数**没做**。它不在返回体里：`ImportRow` 只带邮箱/姓名/总分，
而列名只有在表头与既有不符时才随 `header_warning` 返回。要显示它就得为
一个纯展示的数字再改一次接口形状，而它的信息量远低于同一行的另外两段——
「按什么编码读的」和「少掉的行去哪了」都直接指向"这次是不是出事了"，
列数不指向任何处置。

真要判断"是不是传错了课程"，表头警告那一块给的是**两边的完整列名**，
比一个数字强。所以这条不补。
