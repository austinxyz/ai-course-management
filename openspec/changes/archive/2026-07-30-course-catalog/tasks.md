...
- [x] 7.5 上线顺序 —— **DB → 后端 → 前端**。push 后先确认 `.github/workflows/db-migrate.yml` 的 migration 跑绿，再验收页面；表不存在时后端能正常启动但接口 500，前端会整页错误态
- [x] 7.6 生产验收 —— 建一门真实课程 + 至少两场，其中一场排在 11 月之后；刷新确认落库；确认该场的亚洲时区行与 11 月前那场相差 1 小时
- [x] 7.7 生产验收 —— 给该课程加别名 `S1`（`grades.csv` 现用写法），为下一个 change 的导入匹配做准备
