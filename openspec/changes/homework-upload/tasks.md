...
- [ ] 1.0 CONTRACT — write openspec/changes/homework-upload/contracts/group-1.md with the ### Contract block above; confirm all three fields (Spec, Runtime, Code) are non-empty before proceeding
- [ ] 1.1 GREEN — 把 `tools/homework-sync/parsing.py` 搬到 `backend/app/homework_parsing.py`，`test_parsing.py` 搬到 `backend/tests/test_homework_parsing.py`；只改 import 路径，逻辑一行不动；跑一遍确认既有 18 个用例仍绿（搬迁不是新行为，所以没有 RED 前置）
- [ ] 1.2 RED — 解码：构造一份**真正的 GB18030 字节序列**（`"…".encode("gb18030")`，不是构造 str 再 encode 回来做往返），断言 `decode_csv(raw)` 返回正确中文且报出 `encoding == "gb18030"`
- [ ] 1.3 RED — UTF-8 的字节断言 `encoding == "utf-8"`。只断言"内容对"不够：GB18030 会把 UTF-8 中文解成**另一批中文字**，人眼才分得出，而测试分不出
- [ ] 1.4 RED — 带 BOM 的 UTF-8：第一个列名是 `姓名` 而不是 `﻿姓名`（用 `utf-8-sig` 的理由）
- [ ] 1.5 RED — 两种编码都解不开的字节序列被拒绝，抛出可区分的异常（不是 `BadHeader`）
- [ ] 1.6 GREEN — 实现 `decode_csv(raw: bytes) -> tuple[str, str]`
- [ ] 1.7 RED — 变异验证：把顺序改成先试 GB18030，确认 1.3 **真的变红**；恢复
- [ ] 1.8 RED — 表头缺列的错误要能让人判断"传错文件了"：给一份学员名单形状的 csv，断言错误信息里同时出现「缺了什么」与「这看起来不是作业成绩文件」这类判断依据，而不只是内部列名
- [ ] 1.9 GREEN — 改 `BadHeader` 的信息，并让它携带缺失列名供上层渲染
...
- [ ] 2.0 CONTRACT — write openspec/changes/homework-upload/contracts/group-2.md with the ### Contract block above
- [ ] 2.1 RED — `POST /api/homework/import` 带 base64 的 CSV 字节与 `course_id`，`dry_run=true` 时返回 `created`/`updated`/两份清单/`encoding`/`row_count`，且**库中无任何变化**
- [ ] 2.2 GREEN — 写 migration（两张表 + 索引 + 讲师邮箱回填）与模型；实现 import 端点的 dry-run 路径；`supabase db reset` 后**重启后端进程**（连接池会全废，按 PID 杀）
- [ ] 2.3 RED — `dry_run=false` 真写入，且返回体与 dry-run 的判断一致
- [ ] 2.4 GREEN — 实现写入路径
- [ ] 2.5 RED — 课程入参三种情形：只给 `course_alias` 能解析；两者都给以 `course_id` 为准（构造 alias 指向另一门课，断言写进了 id 那门）；都不给则 4xx 且**一条都不写**
- [ ] 2.6 GREEN — 实现课程解析
- [ ] 2.7 RED — 排除名单：名单中的邮箱不写入、出现在返回体的 `excluded` 里；`POST /api/homework/excluded` 加入一个邮箱后重新 dry-run，`created` 减一
- [ ] 2.8 GREEN — 实现排除名单表与端点
- [ ] 2.9 RED — 排除名单**全课程通用**：某邮箱排除后，导入另一门课的文件同样不写入
- [ ] 2.10 RED — 表头警告：该课程已有 `A1…D2` 的成绩，上传 `E1…G2` 的文件 → 返回体带警告并列出两边列名，但**不拒绝**；该课程一条成绩都没有时**不产生**警告
- [ ] 2.11 GREEN — 实现表头比对
- [ ] 2.12 RED — 导入记录：真写入后产生一条（文件名/编码/行数/新建/更新），dry-run **不产生**；`GET` 该课程最近一次导入信息
- [ ] 2.13 GREEN — 实现导入记录
- [ ] 2.14 RED — 体积上限：超限的请求被拒绝并说明上限，不解析、不写入
- [ ] 2.15 GREEN — 实现上限
- [ ] 2.16 GREEN — 移除 `PUT /api/homework` 及其测试；删除 `tools/homework-sync/` 整个目录，**先把 README 里的教训移进 `homework_parsing.py` 的模块 docstring**
- [ ] 2.17 GREEN — 更新 `backend/tests/conftest.py` 的清表 fixture：两张新表排在 `courses` **之前**删
...
- [ ] 3.0 CONTRACT — write openspec/changes/homework-upload/contracts/group-3.md with the ### Contract block above
- [ ] 3.1 RED — 未带站点密码调用该 Server Action 时被拒绝，且**不发出**任何后端请求
- [ ] 3.2 GREEN — 实现 `previewImport` / `applyImport` 两个 action，开头调 `requireSitePassword()`
- [ ] 3.3 RED — 传给后端的 body 里是 **base64 的原始字节**，不是解码后的字符串。用一份 GB18030 字节夹具断言送出去的内容 base64 解回来与原字节**逐字节相同**
- [ ] 3.4 GREEN — 用 `arrayBuffer()` 而非 `text()`
- [ ] 3.5 RED — 后端返回 4xx 时 action 返回 `{ ok: false, message }` 而**不抛异常**（生产构建里抛出的错误只剩 digest）
- [ ] 3.6 GREEN — 实现错误的返回值表达
- [ ] 3.7 GREEN — `lib/api.ts` 增 `importHomework` / `addExcludedEmail`；确认没有引入任何解析逻辑
- [ ] 3.8 RED — 写入成功后 `revalidatePath("/homework", "layout")` 被调用（页面计数与侧边栏都要跟着变）
- [ ] 3.9 GREEN — 加 revalidate
...
- [ ] 4.0 CONTRACT — write openspec/changes/homework-upload/contracts/group-4.md with the ### Contract block above
- [ ] 4.1 MOCK — 打开 docs/superpowers/specs/mocks/2026-07-31-homework-upload-mocks.html；记下预览屏的结构、编码提示的措辞、表头警告的措辞、以及「这一屏的几条硬规则」那张表
- [ ] 4.2 RED — 预览屏渲染：编码行、行数、去重丢弃、将新建/将更新、两份**分开**的清单（断言两个邮箱之间隔着第二份清单的标题）
- [ ] 4.3 GREEN — 实现 `ImportDialog` 的预览态
- [ ] 4.4 VISUAL DIFF — 起 dev stack，走一遍上传，对照 mock 校 token 与文案；**用 S1 的全量数据**（17 行）确认清单长时不被裁掉
- [ ] 4.5 RED — 确认按钮上写**具体条数**（「确认导入 16 条」），不是「确认」
- [ ] 4.6 GREEN — 实现确认按钮
- [ ] 4.7 RED — 写入期间**所有**出口 disabled（含取消与关闭），用挂住不 resolve 的 promise 断言
- [ ] 4.8 RED — 写入失败时错误就地渲染在预览屏上，且预览屏**仍然打开**
- [ ] 4.9 GREEN — 实现 busy 与错误态
- [ ] 4.10 RED — 标记「以后不算作业」后**发生第二次预览请求**，且界面上的数字来自第二次的返回值（构造两次不同的返回值，断言显示的是第二次那个）
- [ ] 4.11 GREEN — 实现标记与重算
- [ ] 4.12 RED — 编码提示：UTF-8 时也显示；GBK 时显示提醒核对中文
- [ ] 4.13 RED — 表头警告：列出两边列名，确认按钮**仍可用**
- [ ] 4.14 GREEN — 实现编码提示与表头警告
- [ ] 4.15 RED — 作业页：有「导入 grades.csv」按钮、有「上次导入 …」一行、且**没有**逐条编辑/删除/新增成绩的控件
- [ ] 4.16 GREEN — 实现入口与「上次导入」
- [ ] 4.17 VISUAL DIFF — 对照 mock 校编码提示与表头警告两块；确认预览屏在写入中的 disabled 态看得出来
- [ ] 4.18 GREEN — 侧边栏那一项改为「作业 CSV 导入」并真的跳到 `/homework`；删除学员页的「导入 CSV」按钮与新建弹窗里的「批量导入请用 CSV 导入」链接
...
- [ ] 5.2 Run frontend test suite — `cd frontend && npm run test`，确认无回归；单独跑与全量跑结果需一致
- [ ] 5.3 `cd frontend && npx tsc --noEmit` 与 `npx next build`，确认生产构建通过
- [ ] 5.4 Run superpowers:verification-before-completion — 跑 `project.test_commands` 与 `project.custom_verification_checks`；另确认 `tools/homework-sync/` 已不存在且没有任何地方还引用它
