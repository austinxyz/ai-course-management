...
- **Runtime**: `cd backend && uv run pytest tests/test_students_api.py -q` → expected: 既有 100+ 项
  **原样通过**（本 change 不改代码，任何失败都说明动到了不该动的东西）；
  另须确认 `git diff` 在 `backend/` 与 `frontend/` 下为空
- **Code**:
  - **不写代码。** 实现（`backend/app/routers/students.py:46`）与两个测试都已存在
...
- [ ] 1.0 CONTRACT — write openspec/changes/roster-order-spec/contracts/group-1.md with the ### Contract block above
- [ ] 1.1 验证既有测试确实覆盖两条 Scenario —— 逐条比对 `test_list_order_is_stable_across_an_edit`
      与 `test_list_order_breaks_name_ties_deterministically` 的断言，确认 spec 里的两条
      Scenario 不是凭空写的；若有 Scenario 找不到对应测试，**删掉它或补测试**，二选一
- [ ] 1.2 跑 `cd backend && uv run pytest` 确认原样通过
- [ ] 1.3 确认 `git diff --stat backend frontend` 为空 —— 这是本 change 的核心约束
