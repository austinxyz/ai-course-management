...
## Purpose
把系统从公网上关起来：整站以共享密码（HTTP Basic Auth）挡人，后端以共享 secret header 确保只有自家 Next.js 调得通，
生产环境不暴露 API 自动文档。这是一层"防裸奔"级别的保护，不做每人一身份的隔离——讲师与合作伙伴共用同一份凭据。

本能力的两条性质比机制本身更要紧，因此写成 SHALL：**配置缺失时必须拒绝而非放行**，
以及**认证逻辑不含环境判断分支**。理由是失效形状——认证被意外关闭时不会 500、不会报错、页面照常打开，
没有任何外部征兆，可能长期无人察觉；这与"数据库连不上"那类当场暴露的故障完全不同。
## Requirements
