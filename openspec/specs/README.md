...
### `nudge` ✅ 已实现
**用户故事**: 作为讲师，我知道"未交"这个判断系统已经算得出来，但催不催、催了没有全靠我自己记；我想要一个页面告诉我"这门课谁还没交、逾期几天、催过没有"，系统帮我起草好催促文案，我看一眼改改就能发，发完点一下标记，下次不会漏催也不会重复催
**覆盖需求**:
- docs/superpowers/specs/2026-08-03-nudge-requirements.md（MVP：算名单、起草文案、标记已催/跳过、最小互动事件记录；明确不接真实发信、不做多模板、不做导出、不做批量操作）
**设计基准**: docs/superpowers/specs/mocks/2026-08-03-nudge-mocks.html（名单+详情面板，桌面/移动两版；与更早导入的 Claude Design 稿 `ClaudeAI课程学员管理页.dc.html#isNudge` 的差异是刻意的范围裁剪，见该 mock 文件与 proposal 的 Out of Scope）

**后台**: `nudge_events` 表（`student_email`/`course_id`/`event_type` [`nudged`|`skipped`，不建 CHECK、给未来三来源互动记录留口子]/`channel`/`note`/`created_at`），不外键到 `enrollments`——报课记录的生命周期与催促历史无关，与 `homework_submissions` 不外键到 `enrollments` 同一个理由。`GET /api/nudge?course=`：从 `app.routers.homework` 导入 `merge_states`/`state_of` 复用"未交"判定（不重新发明），按（学员, 课程）去重取最早场次日期算逾期天数，`history` 用 Postgres `json_agg` 子查询嵌进主查询一次带出、按时间倒序，跳过的（学员, 课程）用 `WHERE NOT EXISTS` 直接从查询结果里排除；`GET /api/nudge/count`：全课程合计（侧边栏徽标用），同一套逻辑去掉 `course` 过滤、按（学员, 课程）分组，不逐课程调用 `list_nudge` 再相加；`POST /api/nudge/events`：渠道服务端按学员微信是否对齐自动判定（已对齐→微信，否则→邮件），不接受调用方传入
**前台**: `/nudge` 页——课程 tab 切课，表格列姓名/微信（未对齐显示 `Badge variant="danger"`「未对齐微信」，复用学员页视觉语言）/逾期天数/已催次数/上次催促（时间+备注）；选中一人在右侧 360px 详情面板：草稿 `textarea` 自动套姓名/课程/逾期天数（固定模板，无多语气切换），编辑只影响当次展示（挂在 `DetailPanel` 组件自身、靠 `key={studentEmail}` 换人时整个重新挂载天然重置，不提到父组件）；"复制文案"走 `navigator.clipboard`、"标记已催"/"跳过"各自调用 Server Action 并 `revalidatePath("/nudge","layout")`；催促历史区块按时间倒序展示，`channelLabel()` 把 `wechat`/`email` 转成中文"微信"/"邮件"，无历史时显示"还没催过这个人。"
**关键性质**: **名单判据不重新实现**——直接复用 `homework.py` 的 `merge_states`/`state_of`（模块级函数，未加下划线前缀，与 `homework.py` 自己跨路由导入 `courses.py::derive_session_state` 同一先例），两处状态判断分叉的风险等于零。**跳过是查询时排除，不是给报课记录打标记**——`nudge_events` 独立于 `enrollments`/`homework_submissions`，两张源表完全不受影响；已知边界：若某条提交先被跳过、后来又被人工从数据库删掉提交记录导致重新"未交"（`homework_submissions` 没有 API 删除入口，只能直连库），旧的跳过会继续生效，不特别处理。**详情面板的草稿状态不下放到父组件**——最初实现把编辑过的草稿存在父组件的 `Record<email, string>` 里，导致换人后再换回来还看得到上次编辑的文字，违反"编辑只影响当次展示"；正确做法是让状态活在 `DetailPanel` 自己身上，靠 `key` 触发的重新挂载天然复位。**互动记录只建了催作业自己需要的最小子集**——`nudge_events` 表字段（学员+课程+事件类型+时间+渠道+备注）设计上能长成完整的三来源互动记录能力，但这次不建人工录入、不建参与度信号导入的入口
**验收标准**: backend 12 项 + frontend 38 项全绿，四组评审 96/80、100/80、98.4/70（第一次 80/70 被打回，三处渲染缺口：详情面板缺渠道徽章、"上次催促"列缺备注、渠道英文未本地化，修复后二次评审通过）、96/80（第二次评审当场抓到一个真 bug——草稿编辑状态挂在错误的组件层级导致"编辑只影响当次展示"这条 spec 被违反，RED 测试写出来直接复现，修复后验证通过）
