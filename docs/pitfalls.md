...
**"整份导入以文件为准、覆盖式而非同步式删除"这条设计原则，代价是没有任何删除入口**
（homework-auto-create-student，生产事故）。讲师把某门课的作业误传到了另一门课，
自动建档功能把 3 个新学员和 3 条报课都建在了错的课程下。报课记录能修——`enrollments`
有 DELETE/POST，把记录挪到正确课程一分钟搞定。**成绩记录修不了**——`homework_submissions`
唯一写入口是导入本身，没有编辑/删除接口（这是刻意的：两边都能改单条成绩就会分叉），
误导入的那三行只能直接连库 `DELETE`。**症状**：功能上线时测试全绿（正常路径没有删除需求），
直到第一次人为误操作才发现"改错了没有后悔药"。这类"只增不删"的表本身没错，
但引入前要想清楚"写错了怎么办"，而不是等生产事故逼出答案。
...
**写 tasks.md 前要读一遍实现，"这个判断在哪一层算"不能靠猜**（homework-reply-status apply
中途发现）。propose 阶段写"后端 `GET /api/homework` 的「待回复」判据要从 X 改成 Y"，
听着合理、跟 spec 措辞对得上，但实际打开代码一看：那个判据**从来不在后端**，
`list_homework` 只吐原始字段，筛选是 `HomeworkClient.tsx` 里一个纯前端函数
`awaitingReply()` 算的。按计划写下去会给后端端点加一段永远不会被调用路径用到的逻辑，
测试也测不出错——因为没人会去调那条路径断言。**症状**：这类误判不报错、不测试失败，
只是白写一段代码、或者（更隐蔽）在真正该改的地方留了个没打的补丁。发现的时机是
"该写 RED 测试了，去找断言点"，那一步会逼你去读实际调用链，而不是相信自己写的任务描述。

**同一件事有新旧两个信号，只显示旧的会让用户把"新信号没生效"读成"没反应"**（homework-reply-status，
上线当天生产反馈）。列表里"回复"那一列一直显示的是 `grades.csv` 的原始快照；讲师标记
上线后，这一列**没有变**（设计上刻意不碰这一列，避免下次导入覆盖标记）。但对用户来说，
"我点了标记，界面上那格文字纹丝不动"读起来就是"没生效"，即便真正的信号（筛选判据、
详情面板）确实已经更新。**两个独立信号不冲突，不代表用户会分得清哪个信号绑在哪个控件上**——
只要界面上还留着一处显示旧信号，新动作在那里就是"看不见的生效"。修法是让**新信号更可信时
...
**Render 免费档对出站 SMTP 端口（587/465/25）有平台级封锁，跟凭证对不对无关**（nudge-email-send，
生产实测）。本地/CI 测试全程 mock 掉 `smtplib`，看不出这个问题——直到部署上线真的点"发送邮件"，
报错是 `[Errno 101] Network is unreachable`，长得像 DNS/防火墙问题，换账号、换应用专用密码都没用。
真实原因是平台层面不让出站连 SMTP 端口，不是应用代码或凭证的问题。改用走 HTTPS（443）的邮件
API 服务（这里是 Resend）就绕过了这条封锁——**任何要在 Render/类似免费 PaaS 上发邮件的功能，
默认假设 SMTP 走不通，直接选 HTTP API 型的发信服务，不要等部署了才发现**。
